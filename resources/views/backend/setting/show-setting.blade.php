@extends('backend.master')

@section('title')
    Settings
@endsection

@push('page-css')
    {{-- expr --}}
@endpush


@section('content')
    <div class="container-fluid pb-0">
        <div class="card">
            <div class="card-header">
            	<div class="row">
            		<div class="col-md-6">
                		<h2>Settings</h2>
            		</div>
            	</div>

            </div>
            <div class="card-body">
                <div class="col-md-12">
                    <form action="{{ url('/admin/settings/update') }}" method="post" class="contact-form form-group" enctype="multipart/form-data">
                        @csrf
                        <div class="row">
                            <div class="col-md-12">
                                <label for="name">
                                    Logo
                                </label>
                                <input type="file" name="logo" class="form-control" placeholder="Upload Logo">
                                <img src="{{ asset('/logo/'.$setting->logo) }}" height="100px" width="100px" alt="">
                            </div>
                            <div class="col-md-6">
                                <label for="phone">
                                    Phone
                                </label>
                                <input type="text" name="phone" value="{{ $setting->phone }}" class="form-control" placeholder="Enter phone number">
                            </div>
                            <div class="col-md-6">
                                <label for="facebook">
                                    Facebook Link
                                </label>
                                <input type="text" name="facebook" value="{{ $setting->facebook }}" class="form-control" placeholder="Enter facebook link">
                            </div>
                            <div class="col-md-6">
                                <label for="twitter">
                                    Twitter Link
                                </label>
                                <input type="text" name="twitter" value="{{ $setting->twitter }}" class="form-control" placeholder="Enter twitter link">
                            </div>
                            <div class="col-md-6">
                                <label for="linkedin">
                                    LinkedIn Link
                                </label>
                                <input type="text" name="linkedin" value="{{ $setting->linkedin }}" class="form-control" placeholder="Enter linkedin link">
                            </div>
                            <div class="col-md-6">
                                <label for="instagram">
                                    Instagram Link
                                </label>
                                <input type="text" name="instagram" value="{{ $setting->instagram }}" class="form-control" placeholder="Enter instagram link">
                            </div>
                            <div class="col-md-6">
                                <label for="email">
                                    Email
                                </label>
                                <input type="email" name="email" value="{{ $setting->email }}" class="form-control" placeholder="Enter your email" style="background-image: url(&quot;data:image/png;base64,&quot;) !important; background-repeat: no-repeat; background-size: 20px; background-position: 97% center; cursor: auto;" data-temp-mail-org="0">
                            </div>
                            <div class="col-md-12">
                                <label for="address">
                                    Address
                                </label>
                                <input type="text" name="address" value="{{ $setting->address }}" class="form-control" placeholder="Enter your address" style="background-image: url(&quot;data:image/png;base64,&quot;) !important; background-repeat: no-repeat; background-size: 20px; background-position: 97% center; cursor: auto;" data-temp-mail-org="0">
                            </div>
                            <div class="col-md-12">
                                <label for="adsense_code">
                                    Google Adsense Code
                                </label>
                                <textarea name="adsense_code" rows="5" class="form-control" placeholder="Your Google Adsense Code">{{ $setting->adsense_code }}</textarea>
                            </div>
                        </div>
                        <div class="contact-submit-btn-outer">
                            <button class="btn btn-success mt-3">
                                Update <i class="fas fa-paper-plane"></i>
                            </button>
                        </div>
                    </form>
                </div>
            </div>
        </div>
    </div>
@endsection
